...
function StampRow({ count, total }: { count: number; total: number }) {
  return (
    <div className="flex gap-1.5" aria-label={`${count} of ${total} stamps collected`}>
...
export function LoyaltyCardPreview() {
  ...
  // Calculate stamps from visits (simplified: 1 stamp per visit, goal = 10)
  const stampsPerGoal = 10;
  const currentStamps = selectedMember.visits % stampsPerGoal;
  const visitsToFreeReward = stampsPerGoal - currentStamps;
      ...
      <CardContent className="p-5 pt-4 sm:p-6 sm:pt-5">
        <div className="loyalty-card rounded-2xl p-5 sm:p-6">
          <div className="relative z-10 flex items-start justify-between gap-3">
            ...
            <Badge className="h-auto rounded-full border border-line bg-white px-2.5 py-1 text-[9px] font-bold uppercase tracking-[.15em] text-accent-foreground">{selectedMember.tier} tier</Badge>
          </div>
          <div className="relative z-10 mt-8 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-[11px] text-white/60">Member since {new Date(selectedMember.joinedAt).toLocaleDateString("en", { month: "short", year: "numeric" })}</p>
            </div>
          ...
          <div className="relative z-10 mt-6 flex items-center justify-between gap-3">
            <StampRow count={currentStamps} total={stampsPerGoal} />
            <span className="whitespace-nowrap text-[11px] font-bold text-gold">{currentStamps} / {stampsPerGoal} stamps</span>
          </div>
        </div>
