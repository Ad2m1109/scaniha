  ...
  return (
    <div style={style} className={cn("min-h-full overflow-hidden bg-[var(--menu-bg)] text-[var(--menu-text)]", radiusClasses[settings.borderRadius], fontClasses[settings.fontFamily], compact && "text-[90%]")}>
      <header className={cn("relative overflow-hidden border-b border-black/10 px-5 py-8 sm:px-8", settings.headerStyle === "centered" ? "text-center" : "text-left", settings.headerStyle === "split" && "sm:flex sm:items-center sm:justify-between sm:gap-6", headerHasCover && "min-h-52 text-white sm:flex sm:min-h-60 sm:items-end")}>
        {settings.heroImage ? <div className={cn("absolute inset-0 bg-cover bg-center", headerHasCover ? "opacity-100" : "opacity-15")} style={{ backgroundImage: `url(${settings.heroImage})` }} /> : null}
        ...
        <div className={cn("relative", settings.headerStyle === "centered" && "mx-auto", settings.headerStyle === "split" && "max-w-md")}>
          {business.logo ? <div className={cn("mb-3 h-14 w-14 border border-black/10 bg-white bg-cover bg-center", radiusClasses[settings.borderRadius], settings.headerStyle === "centered" && "mx-auto")} style={{ backgroundImage: `url(${business.logo})` }} /> : <div className={cn("mb-3 grid h-12 w-12 place-items-center bg-[var(--menu-accent)] text-lg font-bold text-white", radiusClasses[settings.borderRadius], settings.headerStyle === "centered" && "mx-auto")}>{business.name.charAt(0)}</div>}
          ...
          <h1 className="mt-2 text-2xl font-bold tracking-tight">{business.name}</h1>
      ...
      <main className={cn("px-4 py-6 sm:px-7", settings.layout === "compact" ? "space-y-5" : "space-y-8")}>
        {[...categories].sort((a, b) => a.sortOrder - b.sortOrder).map((category) => {
          ...
          return <section key={category.id}>
            <div className={cn("mb-3", settings.categoryStyle === "underline" && "border-b-2 border-[var(--menu-accent)] pb-2", settings.categoryStyle === "filled" && "bg-[var(--menu-accent)] px-3 py-2 text-white", settings.categoryStyle === "filled" && radiusClasses[settings.borderRadius])}>
            ...
            <div className={cn("grid", settings.layout === "grid" ? "gap-3 sm:grid-cols-2" : settings.layout === "compact" ? "divide-y divide-black/10" : "gap-3")}>
              {items.map((product) => <article key={product.id} className={cn("flex gap-3", settings.layout === "compact" ? "py-2" : "p-3", settings.cardStyle !== "minimal" && "bg-[var(--menu-card)]", settings.cardStyle === "elevated" && "border border-black/5 shadow-sm", settings.cardStyle === "outline" && "border border-black/15", settings.cardStyle !== "minimal" && radiusClasses[settings.borderRadius])}>
                {settings.showImages && product.image ? <div className={cn("shrink-0 bg-cover bg-center", settings.layout === "compact" ? "h-12 w-12" : "h-16 w-16", radiusClasses[settings.borderRadius])} style={{ backgroundImage: `url(${product.image})` }} /> : null}
                <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><h3 className="text-xs font-bold">{product.name}</h3><span className="shrink-0 text-xs font-bold text-[var(--menu-accent)]">{settings.currency} {product.price.toFixed(2)}</span></div>{settings.showDescriptions && product.description ? <p className="mt-1 text-[10px] leading-relaxed text-[var(--menu-muted)]">{product.description}</p> : null}</div>
              </article>)}
            </div>
          </section>;
        })}
        {!available.length ? <div className="py-16 text-center"><p className="font-bold">The menu is being prepared.</p><p className="mt-1 text-xs text-[var(--menu-muted)]">Please check back soon.</p></div> : null}
      </main>

      {settings.showContactInfo ? <footer className="border-t border-black/10 px-5 py-5 text-center text-[10px] text-[var(--menu-muted)]"><div className="flex flex-wrap justify-center gap-4">{business.phone ? <a href={`tel:${business.phone}`} className="flex items-center gap-1"><Phone className="h-3 w-3" />Call</a> : null}{business.instagram ? <span className="flex items-center gap-1"><Instagram className="h-3 w-3" />{business.instagram}</span> : null}</div><p className="mt-3">Powered by Perkly</p></footer> : null}
    </div>
  );
}
