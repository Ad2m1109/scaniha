...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Card className={cn("glass-card min-w-0 overflow-hidden border-0 p-0 ring-0", !product.available && "opacity-60")}>
      {product.image ? <div className="h-32 bg-cover bg-center" style={{ backgroundImage: `url(${driveImage(product.image)})` }} /> : <div className="grid h-24 place-items-center bg-surface-soft"><ImageIcon className="h-6 w-6 text-faint" /></div>}
