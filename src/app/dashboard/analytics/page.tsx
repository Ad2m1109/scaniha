...
export default function AnalyticsPage() {
  ...
  return <div className="mx-auto max-w-[1280px] space-y-4"><PageIntro eyebrow="Performance" title="Know what's working at the counter." description="Measure menu discovery and loyalty activity using real actions in this workspace." action={<div className="flex rounded-xl border border-line bg-surface-soft p-1">{(["7D", "30D", "90D"] as ChartRange[]).map((item) => <Button key={item} variant="ghost" size="sm" className="range-button h-8" data-active={range === item} onClick={() => setRange(item)}>{item}</Button>)}</div>} /><section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{stats.map((stat) => { const Icon = stat.icon; return <Card key={stat.label} className="glass-card border-0 ring-0"><CardContent className="flex items-center gap-3 p-4"><span className={`grid h-9 w-9 place-items-center rounded-xl ${stat.style}`}><Icon className="h-4 w-4" /></span><div><p className="metric-value text-lg">{formatNumber(stat.value)}</p><p className="mt-0.5 text-[11px] text-muted">{stat.label}</p></div></CardContent></Card>; })}</section><MemberActivityChart key={range} defaultRange={range} /></div>;
}
