...
export default function MenuPage() {
  ...
  return <div className="mx-auto max-w-[1280px] space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]"><CategoryList /><section aria-label="Menu items" className="grid grid-cols-1 gap-3 md:grid-cols-2">{ready && products.map((product) => <ProductCard key={product.id} product={product} />)}{ready && !products.length ? <div className="glass-card col-span-full rounded-2xl p-10 text-center"><p className="font-bold text-ink">Your menu is empty</p><p className="mt-1 text-xs text-muted">Add your first category and product to publish a menu.</p></div> : null}</section></div>
  </div>;
}
